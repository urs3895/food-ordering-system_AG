...
public class PaymentRequestKafkaListener implements KafkaConsumer<PaymentRequestAvroModel> {
    ...
    public void receive(List<ConsumerRecord<String, PaymentRequestAvroModel>> records) {
        log.info("{} number of payment requests received", records.size());

        for (ConsumerRecord<String, PaymentRequestAvroModel> record : records) {
            String key = record.key();
            int partition = record.partition();
            long offset = record.offset();
            PaymentRequestAvroModel paymentRequestAvroModel = record.value();
